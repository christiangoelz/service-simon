...
SECRETS = [{'minimum':1, 'maximum':3}, {'minimum':2, 'maximum':4}, {'minimum':7, 'maximum':9}]
...
if __name__ == "__main__":
    ...
    MY_SECRET = SECRETS[MY_INDEX]  # my secret input

    api = federatedsecure.client.Api(MY_NODE)
    microservice = api.create(protocol="Simon")
    result = microservice.compute(microprotocol="MinimumMaximum", data=MY_SECRET, network=MY_NETWORK)
